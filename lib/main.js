  ...
  async clickCoinButton() {
    ...
    if (obj) { 
        ...
        if (count % 20 === 0) { 
            ...
            // Show transaction message on the UI
            const transactionMessage = document.createElement("div");
            transactionMessage.innerText = "Transaction Sent!";
            transactionMessage.className = "transaction-message";
            document.body.appendChild(transactionMessage);
            setTimeout(() => transactionMessage.remove(), 2000); // Remove after 2 seconds
        }
    }
  }
